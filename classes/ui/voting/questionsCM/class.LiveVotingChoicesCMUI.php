...
class LiveVotingChoicesCMUI
{
    ...
    {
        if ($result && isset($result["config_question"], $result["config_answers"]["hidden"]) && $result["config_answers"]["hidden"] !== "" && $result["config_score"]["points"] > 0) {

            $question_data = $result["config_question"];
            $scoring_data = $result["config_score"];
